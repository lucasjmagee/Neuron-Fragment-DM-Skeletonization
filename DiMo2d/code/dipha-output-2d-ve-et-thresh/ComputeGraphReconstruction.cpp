...
vector<int> bfs(int vert, int &branch_min, vector<int> vert_arr[], bool visited[], const vector<int> neighborhoods[], int pre[]){
	...
	while (!q.empty())
	{
		...
		int current_val = vert_arr[current_vert][2];
		if (current_val < vert_arr[branch_min][2])
		{
		...
		else if (current_val == vert_arr[branch_min][2]  && current_y < vert_arr[branch_min][1])
		{
			branch_min = current_vert;
		}
		else if (current_val == vert_arr[branch_min][2] && current_y == vert_arr[branch_min][1] && current_x < vert_arr[branch_min][0])
		{
			branch_min = current_vert;
		}
...
int main(int argc, char* argv[])
{
	...
	int ve_persistence_threshold = atoi(argv[3]);
	int et_persistence_threshold = atoi(argv[4]);
	...
	vector<vector<int> > vf_edges;
		...
		if (persistence > ve_persistence_threshold)
		{
			continue;
		}
	...
	*/
	...
	int know_min = 0;
		...
		else
		{
			type = 1;
                ...
                if (type == 0  && persistence <= ve_persistence_threshold)
                {
			/*
			clock_t d_end = clock();
		...
		if (type == 1 && persistence <= et_persistence_threshold)
		{
			continue;
		}
	...
	string vertex_filename = output_dir + "dimo_vert.txt";
        ...
        for (int i = 0; i < output_verts.size(); i++)
        {
                ...
                vFile << vert[0] << " " << vert[1] << " " << vert[2] << endl;
        }
