...
def write_vtp_graph(input_dir, output_filename, ve_persistence_threshold=0, et_persistence_threshold=64):
    ...
    for image_filename in image_filenames:
        ...
        with open(vert_filename, 'r') as vert_file:
            reader = csv.reader(vert_file, delimiter=' ')
            for row in reader:
                pts.InsertNextPoint(.92 * int(row[1]), .92 * int(row[0]), 40 * plane_count)
        ...
        with open(edge_filename, 'r') as edge_file:
            ...
            for row in reader:
                ...
                new_line.GetPointIds().SetId(0, vert_count + int(row[0]))
                new_line.GetPointIds().SetId(1, vert_count + int(row[1]))
                lines.InsertNextCell(new_line)
            edge_file.close()

        vert_count += local_count
        plane_count += 1
    ...
    linesPolyData.GetPointData().SetScalars(ns.numpy_to_vtk(weights))

    linesPolyData.SetLines(lines)

    gw = vtk.vtkXMLPolyDataWriter()
    gw.SetFileName(output_filename)
